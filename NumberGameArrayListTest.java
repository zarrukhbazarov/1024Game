...
public class NumberGameArrayListTest {
...
//                    after[c.row][c.column] != 0);
    ...
    @Test(timeout = 5000)
    public void testIsGameBoardFullNoMoreMoves() {
    ...
    private void renderBoard(int n) {
        int[][] grid = new int[n][n];

        int CELL_WIDTH = 3;
        String NUM_FORMAT = String.format("%%%dd", CELL_WIDTH + 1);

        /* Set the string to %4s, but without using String.format() */
        String BLANK_FORMAT = "%" + (CELL_WIDTH + 1) + "s";
